...
const { liriklagu, quotemaker, wall, costom } = require('./lib/functions')
...
module.exports = msgHandler = async (client, message) => {
    try {
		...
		if (body.includes('chat.whatsapp.com')) {
			await client.removeParticipant(chatId, sender.id)
		}
        ...
        const botadmins = ['3496349190@c.us'] //add the number of people that you want to the be bot admins
       ...
       case 'vote' :
            ...
            break
       case 'resetpoll':
            feature.adminpollreset(client, message, message.body.slice(10), pollfile, voterslistfile)
            break
       case 'add': 
            feature.addcandidate(client, message, message.body.slice(4), pollfile, voterslistfile)
            break
       case 'translate':
            arg = body.trim().split(' ')
